...
import org.ohmj.core.BloomFilter;
import org.ohmj.core.ECKey;
import org.ohmj.core.NetworkParameters;
import org.ohmj.core.Utils;
import org.ohmj.crypto.*;
import org.ohmj.script.Script;
import org.ohmj.utils.Threading;
 ...
 * {@link org.ohmj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.ohmj.crypto.MnemonicCode},
 ...
 * {@link org.ohmj.crypto.DeterministicKey#serializePubB58(org.ohmj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.ohmj.crypto.DeterministicKey#deserializeB58(org.ohmj.crypto.DeterministicKey, String, org.ohmj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.ohmj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.ohmj.crypto.DeterministicHierarchy} and
 * {@link org.ohmj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final ChildNumber OHM_PATH = new ChildNumber(119,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, OHM_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_OHM_ONLY)
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_OHM_ONLY){
    ...
    public List<DeterministicKey> getKeys(KeyPurpose purpose, int numberOfKeys) {
        ...
        try {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                // by any real problem reports from ohmj users, but I've heard of cases via the grapevine of
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.ohmj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_OHM_ONLY;
                break;
            }
        }
     ...
     * @see org.ohmj.wallet.MarriedKeyChain
     */
