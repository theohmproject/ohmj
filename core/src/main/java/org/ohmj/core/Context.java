...
import org.ohmj.core.listeners.BlockChainListener;
import org.ohmj.store.FlatDB;
import org.ohmj.store.MasternodeDB;
 ...
 * ohmj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating ohmj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use ohmj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a ohmj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using ohmj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing ohmj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.ohmj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.ohmj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.ohmj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initConcierge(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### Concierge init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented concierge MN stuff
