...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See ohmj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.ohmj.core.NetworkParameters}.
     ...
     *
     * <p>The balance {@link org.ohmj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
